...
def CreateDatasets(dataset_dict, hdx_site, apikey, verbose=True, update_all_datasets=False):
  ...
  #
  i = 0
  widgets = [I('bullet'), ' Creating datasets:', pb.Percentage(), ' ', pb.Bar('-'), ' ', pb.ETA(), ' ']
  ...
  for dataset in dataset_dict:
    ...
    if check["success"] is True and check['result']['state'] != "deleted":
      ...
      if update_all_datasets is True:
        ...
        i += 1
        if verbose:
          print("%s Dataset exists. Skipping %s" % (I('warn'), dataset["name"]))
        continue
    ...
    #
      ...
      if r.status_code != 200:
        ...
        if verbose:
          print("%s created successfully %s" % (I('success'), dataset["name"]))
...
def CreateResources(resource_dict, hdx_site, apikey, verbose=True, update_all_datasets=False):
  ...
  #
  i = 0
  widgets = [I('bullet'), ' Creating resources:', pb.Percentage(), ' ', pb.Bar('-'), ' ', pb.ETA(), ' ']
  ...
  for resource in resource_dict:
    ...
    if resource["format"] is None:
      if verbose:
        print("%s skipping %s" % (I('warn'), resource["name"]))
      continue
    ...
    #
      ...
      if resource['name'] in names_array:
        i += 1
        if verbose:
          print("%s Resource exists. Skipping %s" % (I('warn'), resource["name"]))
        continue
    ...
    if r.status_code != 200:
      ...
      if verbose:
        print("%s created successfully %s" % (I('success'), resource["name"]))
...
def CreateGalleryItems(gallery_dict, hdx_site, apikey, verbose=True, update_all_datasets=False):
  ...
  #
  i = 0
  widgets = [I('bullet'), ' Creating gallery items:', pb.Percentage(), ' ', pb.Bar('-'), ' ', pb.ETA(), ' ']
  ...
  for item in gallery_dict:
    ...
    if dataset["success"] is True:
      ...
      if len(old_related["result"]) > 1:
        print("%s Dataset %s has more than one gallery item. Please check manually." % (I('bullet'), item["dataset_id"]))
        continue
      ...
      def _delete_old(old_array):
        ...
        for result in old_array["result"]:
          ...
          if verbose:
            print("%s Existing gallery item. Deleting. %s" % (I('warn'), result["id"]))
      ...
      if update_all_datasets:
        ...
        if item['url'] in related_array:
          i += 1
          if verbose:
            print("%s Gallery item exists. Skipping. %s" % (I('warn'), item["url"]))
          continue
          ...
          _delete_old(old_related)
          r = requests.post(related_create_url, data=json.dumps(item), headers=headers, auth=('dataproject', 'humdata'))
          print("%s Creating gallery item. %s" % (I('bullet'), item["url"]))


      if r.status_code != 200:
        if verbose:
          print("%s failed to create %s" % (I('error'), item["url"]))
          print(r.json())

      else:
        if verbose:
          print("%s created successully %s" % (I('success'), item["url"]))
