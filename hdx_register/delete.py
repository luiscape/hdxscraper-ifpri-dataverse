...
def DeleteAllDatasetsFromOrg(organization, hdx_site, apikey, verbose=True):
  ...
  #
  i = 0
  widgets = [I('bullet'), ' Deleting resources:', pb.Percentage(), ' ', pb.Bar('-'), ' ', pb.ETA(), ' ']
  ...
  if dataset_dict["success"] is True:
    ...
    for dataset in dataset_dict["result"]["packages"]:
      ...
      if r.status_code != 200:
        print("%s : %s" % (I('error'), dataset["name"]))

      else:
        print("%s : %s" % (I('success'), dataset["name"]))

    i += 1

  else:
    print("%s There was an error getting the dataset list." % I('error'))
    print("--------------------------------------------------")
    return False
...
def DeleteResources(dataset_dict, hdx_site, apikey, verbose=True):
  ...
  if (dataset_dict is None):
    print("%s No data provided. Provide a JSON package." % I('error'))
    print("--------------------------------------------------")
    return
  ...
  for dataset in dataset_dict:
    ...
    if d["success"] is False:
      if d['error']['__type'] == 'Not Found Error':
        print('%s Dataset not found.' % I('warn'))
      else:
        print('%s There was an error connecting to HDX.' % I('error'))
        if verbose:
    ...
    if d["success"] is True:
      for resource in d["result"]["resources"]:
        if verbose:
          print("%s : resource deleted %s" % (I('warn'), resource["id"]))
